 ...
 class PaygentB2BModuleResources {
	...
	static function &getInstance($app) {
		...
		if (isset($resourceInstance) == false
			...
			$resourceInstance = new PaygentB2BModuleResources($app);
	...
	function readProperties() {
		...
		$prop = PaygentB2BModuleResources::parseJavaProperty(dirname(__FILE__) . $this->app['const']['PaygentB2BModuleResources__PROPERTIES_FILE_NAME']);
		if ($prop === false) {
		...
		if (array_key_exists($this->app['const']['PaygentB2BModuleResources__CLIENT_FILE_PATH'], $prop)
			...
			$this->clientFilePath = $prop[$this->app['const']['PaygentB2BModuleResources__CLIENT_FILE_PATH']];
		}
		...
		if (array_key_exists($this->app['const']['PaygentB2BModuleResources__CA_FILE_PATH'], $prop)
			...
			$this->caFilePath = $prop[$this->app['const']['PaygentB2BModuleResources__CA_FILE_PATH']];
		}
		...
		// UTF-8の電文種別リスト
		$keyUtf8 = $this->app['const']['PaygentB2BModuleResources__TELEGRAM_KIND_UTF8'];
		if (array_key_exists($keyUtf8, $prop) && !StringUtil::isEmpty($prop[$keyUtf8])) {
		    $this->telegramKindUtf8s = $this->split($prop[$keyUtf8], $this->app['const']['PaygentB2BModuleResources__TELEGRAM_KIND_SEPARATOR']);
		}
